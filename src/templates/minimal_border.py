"""
Template D: Minimal Border
Minimal white border, small EXIF text — for social media.
"""
...
class MinimalBorderTemplate(BaseTemplate):
    name = 'Minimal Border'
    description = 'Minimal white border, tiny EXIF — Instagram / social'
    ...
    def render(self, image: Image.Image, exif: ExifSource, params: RenderParams) -> Image.Image:
        ...
        bg_color, font_color = template_colors(params, (255, 255, 255), (34, 34, 34))
        
        m_top = int(max(params.margin_top, 34) * canvas_h / 2048)
        m_side = int(max(params.margin_side, 34) * canvas_w / 2048)
        m_bottom = int(max(params.margin_bottom, 76) * canvas_h / 2048)
        ...
        if exif_line and params.font_size > 0:
            text_y = img_y + new_h + int(m_bottom * 0.25)
            ...
            self._draw_fitted_text(
                ...
                max(8, params.font_size - 4), font_color,
